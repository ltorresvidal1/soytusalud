...
const TrabajaNosotros = () => {
	...
	const handleSubmit =async(e)=>{
		...
		e.preventDefault();
		const perfil = ref(storage,`servicios${authUser.identificacion}/foto.jpg`)
		const distintivoHabilitacion = ref(storage,`servicios/${authUser.identificacion}/distintivoHabilitacion.pdf`)
		const convalidaciónICFES = ref(storage ,`servicios/${authUser.identificacion}/convalidaciónICFES.pdf`)
		const fotoLogoPublicidad = ref(storage ,`servicios/${authUser.identificacion}/fotoLogoPublicidad.jpg`)
		const hojaVida = ref(storage,`servicios/${authUser.identificacion}/hojaVida.pdf`)

		await uploadBytes(perfil,formData.perfil)
		await uploadBytes(distintivoHabilitacion,formData.distintivoHabilitacion)
		await uploadBytes(convalidaciónICFES,formData.convalidaciónICFES)
		await uploadBytes(fotoLogoPublicidad,formData.fotoLogoPublicidad)
		await uploadBytes(hojaVida,formData.hojaVida)

		await getDownloadURL(perfil)
		.then(url=>{
			formData.perfil=url
		})
		...
		await getDownloadURL(convalidaciónICFES)
		.then(url=>{
			formData.convalidaciónICFES=url
		})
								...
								<div>
									<div className="col-xl-12">
										<form className="form message-form" ref={form} onChange={updateFormData} onSubmit={handleSubmit}  id="Form_TuHistoria" >
												...
												<label className="control-label mb-1">Por favor, verifique los campos obligatorios marcados con un (*) </label>   
													...
													<div className="row">
														...
														<div className="col-lg-3 mt-3">
															...
															<input className="form-control" type="url" name="web" id="web" />
														</div>
														<div className="col-lg-3 mt-3">
											...
											<div onClick={()=>{setServicios([...servicios,servicios.length+1])}} className="relative cursor-pointer add-button w-6 h-6 rounded-full"><i className="p-2 fas fa-plus fa-xs"></i></div> 
											<div className="row">
													...
													<div key={index}>
														...
														<div className="row">
															<div className="col-lg-2">
																...
																<input type="time" className="form-control" name={`disponibilidadHoraria${index}`} id="disponibilidadHoraria"/>
															</div>
															<div className="col-lg-2">
															...
															<div className="col-lg-8">
																...
																<div className="col-12 space-x-2">
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text " value={true}>Lunes</span>
																	...
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text" value={true}>Martes</span>
																		<input className="form__input-checkbox" type="checkbox" name="recopilacionDatos" id="recopilacionDatos" required/>
																		<span className="form__checkbox-mask"></span>
																	</label>
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text" value={true}>Mircoles</span>
																		<input className="form__input-checkbox" type="checkbox" name="recopilacionDatos" id="recopilacionDatos" required/>
																		<span className="form__checkbox-mask"></span>
																	</label>
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text" value={true}>Jueves</span>
																		<input className="form__input-checkbox" type="checkbox" name="recopilacionDatos" id="recopilacionDatos" required/>
																		<span className="form__checkbox-mask"></span>
																	</label>
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text" value={true}>Viernes</span>
																		<input className="form__input-checkbox" type="checkbox" name="recopilacionDatos" id="recopilacionDatos" required/>
																		<span className="form__checkbox-mask"></span>
																	</label>
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text" value={true}>Sabado</span>
																		<input className="form__input-checkbox" type="checkbox" name="recopilacionDatos" id="recopilacionDatos" required/>
																		<span className="form__checkbox-mask"></span>
																	</label>
																	<label className="form__checkbox-label" >
																		<span  id="recopilacionDatos" className="form__label-text" value={true}>Domingo</span>
																		<input className="form__input-checkbox" type="checkbox" name="recopilacionDatos" id="recopilacionDatos" required/>
																		<span className="form__checkbox-mask"></span>
																	</label>
																</div>
														...
														<div className='row'>
															<div className="col-lg-4">
																...
																<input className="form-control" name={`valorServicio${index}`} id="valorServicio" required></input>
															</div>
														</div>
													</div>
													)})} 
											...
											<div>
												...
												<div className="row">
													...
													<div className="col-lg-4">
														...
														<label className="space-x-2" htmlFor="convalidaciónICFES">
															<svg xmlns="http://www.w3.org/2000/svg" className="iborrainputfile" width="20" height="17" viewBox="0 0 20 17">
																<path d="M10 0l-5.2 4.9h3.3v5.1h3.8v-5.1h3.3l-5.2-4.9zm9.3 11.5l-3.2-2.1h-2l3.4 2.6h-3.5c-.1 0-.2.1-.2.1l-.8 2.3h-6l-.8-2.2c-.1-.1-.1-.2-.2-.2h-3.6l3.4-2.6h-2l-3.2 2.1c-.4.3-.7 1-.6 1.5l.6 3.1c.1.5.7.9 1.2.9h16.3c.6 0 1.1-.4 1.3-.9l.6-3.1c.1-.5-.2-1.2-.7-1.5z"></path>
															</svg>
															<span className="iborrainputfile">Seleccionar archivo</span>
												...
												<div className="row mt-4">
													<div className="col-lg-12">
														...
														<textarea required className="form-control" name="resumenDeCurriculum" id="resumenDeCurriculum" placeholder=""></textarea>
													</div>
												</div>
												...
												<div className="row mt-10">
													<div className=" flex flex-col ml-3 mt-12">
														<label className="form__checkbox-label"><span name="convenioProfesionales" id="convenioProfesionales" className="form__label-text">Acepta <a href="https://firebasestorage.googleapis.com/v0/b/quetions-app.appspot.com/o/Terminos%2FContrato%20de%20acceso%20a%20la%20Plataforma%20Fundaci%C3%B3n%20Soy%20T%C3%BA%20Salud%20Profesionales.pdf?alt=media&token=6de426d2-45a9-4edd-9b1c-bab313aaca60" target='_blank' required rel="noreferrer">Convenio Profesionales</a></span>
															<input className="form__input-checkbox" type="checkbox" name="checkConvenioProfesionales" id="checkConvenioProfesionales" value="1"/>
															<span className="form__checkbox-mask"></span>
														</label>
														<label className="form__checkbox-label"><span name="convenioProveedores" id="convenioProveedores" className="form__label-text">Acepta <a href="https://firebasestorage.googleapis.com/v0/b/quetions-app.appspot.com/o/Terminos%2FContrato%20de%20acceso%20a%20la%20Plataforma%20Fundaci%C3%B3n%20Soy%20T%C3%BA%20Salud-Proveedores.pdf?alt=media&token=df28ca97-f067-44a2-8645-09eff342cdac" target='_blank' required rel="noreferrer">Convenio Proveedores</a></span>
															<input className="form__input-checkbox" type="checkbox" name="checkConvenioProveedores" id="checkConvenioProveedores" value="1"/>
															<span className="form__checkbox-mask"></span>
														</label>
														<label className="form__checkbox-label"><span name="politicaDatos" id="politicaDatos" className="form__label-text">Acepta Política de <a href="https://firebasestorage.googleapis.com/v0/b/quetions-app.appspot.com/o/Terminos%2FPol%C3%ADtica%20de%20tratamiento%20de%20datos%20Fundaci%C3%B3n%20Soy%20T%C3%BA%20Salud.pdf?alt=media&token=5b6dc7da-566a-4bff-8b19-87ceaafed660" target='_blank' required rel="noreferrer">Tratamiento de Datos Personales</a></span>
															<input className="form__input-checkbox" type="checkbox" name="checkPoliticaDatos" id="checkPoliticaDatos" value="1"/>
															<span className="form__checkbox-mask"></span>
														</label>
														<label className="form__checkbox-label"><span name="docSarlaft" id="docSarlaft" className="form__label-text">Acepta documento <a href="https://firebasestorage.googleapis.com/v0/b/quetions-app.appspot.com/o/Terminos%2FPol%C3%ADticas%20para%20el%20Sistema%20de%20Administraci%C3%B3n%20de%20Riesgos%20de%20Lavado%20de%20Activos%20y%20Financiaci%C3%B3n%20del%20terrorismo%20Fundaci%C3%B3n%20Mi%20Salud%20PLUS.pdf?alt=media&token=07652efc-1eda-484e-b5f4-fec8bc9e3bcc" target='_blank' required rel="noreferrer">SARLAFT </a>de Fundación Soy Tú Salud</span>
															<input className="form__input-checkbox" type="checkbox" name="checkDocSarlaft" id="checkDocSarlaft" value="1"/>
															<span className="form__checkbox-mask"></span>
														</label>
